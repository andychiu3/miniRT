...
/*   Updated: 2025/01/17 11:28:34 by fiftyblue        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	intersect_cy_cap(t_ray *ray, t_cy *cy)
{
	...
	t1 = plane_formula(ray, cy->btm_c, cy->vec);
	t2 = plane_formula(ray, cy->top_c, cy->vec);
	t = -1.0;
	if (in_circle_range(ray, t1, cy->btm_c, cy->diameter / 2))
		t = t1;
	if (in_circle_range(ray, t2, cy->top_c, cy->diameter / 2))
		if (t < 0 || t2 < t)
			t = t2;
	return (t);
}
